...
if (prevButton && nextButton && dateElement && displayElement && dropdownList) {
	...
	let currentDate = new Date(displayElement.getAttribute("data-value") || today.toDateString());
	...
	function updateButtonsState() {
		...
		const referenceDate = getDateWithoutTime(new Date(displayElement.getAttribute("data-value")));
	...
	function updateActiveDate() {
		Array.from(dropdownList.children).forEach((item) => {
			item.classList.toggle("active", item.getAttribute("data-value") === formatDateValue(currentDate));
		});
	}
	...
	function generateDateList() {
		...
		for (let date = startDate; date <= endDate; date.setDate(date.getDate() + 1)) {
			...
			listItem.textContent = date.toDateString() === today.toDateString() ? "Today" : formatDateDisplay(date);
