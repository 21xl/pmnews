...
export function registerHandlebarsHelpers() {
	...
	Handlebars.registerHelper("isHomeScoreGreater", (homeScores) => Array.isArray(homeScores) && homeScores[2] > 0);
	...
	Handlebars.registerHelper("winnerClass", (homeScores, awayScores) => (homeScores > awayScores ? "winner" : ""));

	Handlebars.registerHelper("displayScore", (status, scores) => {
		const statusesWithDash = ["1", "9", "10", "11", "12", "13"];
		return statusesWithDash.includes(String(status)) ? "-" : (scores?.[0] ?? "-");
	});
	...
	Handlebars.registerHelper("finishedMatchTimeSingle", function (match) {
		...
		const dateStr = `${kickoffDate.getDate().toString().padStart(2, "0")}.${(kickoffDate.getMonth() + 1)
			.toString()
			.padStart(2, "0")}`;
	...
	Handlebars.registerHelper("matchTimeOrBreak", function (matchId) {
		...
		if (!data || typeof data !== "object" || !data["0"] || !Array.isArray(data["0"].matches)) {
	...
	Handlebars.registerHelper("matchTimeOrBreakSingle", function (match) {
		...
		const kickoffTimestamp = match.match_time; // Используем match_time из ваших данных
		const currentTimestamp = Math.floor(Date.now() / 1000);
		const matchElement = document.querySelector(`[data-match_id="${match.id}"]`);
		const startTime = new Date(kickoffTimestamp * 1000);
		const today = new Date();
	...
	Handlebars.registerHelper("isMatchEnded", (statusId) => ["100"].includes(String(statusId)));

	Handlebars.registerHelper("isBall", (serving_side, data) => (serving_side === data ? "ball" : ""));
...
Handlebars.registerHelper("ifLogo", function (data) {
	...
	return "/wp-content/themes/pm-news/sport/src/img/world.svg";
});
