...
import templateSource from "../../templates/calendar-template.html";
...
jQuery(document).ready(function ($) {
  ...
  const cateoryId = $(".matches-tennis__main").data("id");
  ...
  favoriteTeamsStore.onDataChange((teams) => {
    console.log("Favorite teams updated:", teams);

    // Применяем шаблон к каждой команде и собираем HTML
    const newHtml = teams.map((team) => favTemplate(team)).join("");
  ...
  function renderMatches(data) {
    console.log("renderMatches", data);
  ...
  function renderMatchesWithoutSorting() {
    const data = calendarStore.getData();
    renderMatches(data);
  }
  ...
  async function fetchAndRenderMatches() {
    try {
      const response = await fetch(
        `${apiUrl}/api/tennis/tournaments/by-category?category_id=${cateoryId}`
      );
      if (!response.ok)
        throw new Error(`Ошибка запроса: ${response.statusText}`);
      const { data } = await response.json();
      calendarStore.setData(data);
  ...
  if (matchesMain && cateoryId) fetchAndRenderMatches();
  ...
  socket.onmessage = (event) => {
    ...
    data.forEach((matchUpdate) => {
      if (matchUpdate.score) calendarStore.updateMatchData(matchUpdate.score);
      if (matchUpdate.incidents) updateIncident(matchUpdate);
    });
  };
  socket.onclose = () => {};

  calendarStore.onDataChange(({ updatedMatch }) => {
