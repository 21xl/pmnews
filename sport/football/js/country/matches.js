...
jQuery(document).ready(function ($) {
  ...
  async function fetchAndRenderMatches(
    ...
    try {
      const response = await fetch(
        `/wp-json/sports/v1/matches_by_location?data=${id}&status=${tab}&location=${locType}&date=${localFormattedDate}`
      );
  ...
  async function onDateChange(newDateValue) {
    await fetchAndRenderMatches(locId, tab, newDateValue);

    // const tabElement = document.querySelector(
    //   `.tabs__item[data-status="sheduled"]`
    // );
    // if (tab == "ended") return;
    // if (tabElement) {
  ...
  if (matchesMain && locId) {
    fetchAndRenderMatches(locId);
  }
