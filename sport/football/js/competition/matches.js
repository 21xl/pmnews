  ...
  function renderMatches(data) {
    if (data.matches?.length > 0 && data.matches[0]?.round?.round_num !== 0) {
      ...
      const groupedMatches = data.matches.reduce((acc, match) => {
        if (match.status_id >= 8) return acc;
      ...
      data.rounds = Object.values(groupedMatches)
        .map((roundGroup) => {
          roundGroup.matches.sort((a, b) =>
            (a.status_id >= 1 && a.status_id <= 7) !==
            (b.status_id >= 1 && b.status_id <= 7)
              ? a.status_id >= 1 && a.status_id <= 7
                ? -1
                : 1
        ...
        .sort((a, b) => a.round.round_num - b.round.round_num);
      ...
      data.matches = data.matches
        .filter((match) => match.status_id < 8)
        .sort((a, b) =>
          (a.status_id >= 1 && a.status_id <= 7) !==
          (b.status_id >= 1 && b.status_id <= 7)
            ? a.status_id >= 1 && a.status_id <= 7
      ...
      if (data.matches.length == 0) matchesMain.html(mEerortemplate);
      else matchesMain.html(template(data));
    }
