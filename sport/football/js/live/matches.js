...
jQuery(document).ready(function ($) {
  ...
  function updatePinnedItems(id) {
    ...
    const item = competitionStore
      .getData()
      .find((competition) => competition.competition.id === id);

    if (item) {
      pinned.updatePinned(item);
    } else {
      if (index !== -1) {
        pinned.delPinned(id);
      }
  ...
  function renderMatches(data) {
    const matches =
      data.matches?.filter(
        (match) => match.status_id <= 7 && match.status_id > 1
      ) || [];
    const curRound = data.competition?.cur_round ?? 0;

    if (!matches.length) return matchesMain.html(mEerortemplate);

    if (matches.length && data.matches[0]?.round?.round_num !== 0) {
      const groupedMatches = matches.reduce((acc, match) => {
        let round;
        if (typeof match.round === "string") {
          try {
            round = JSON.parse(match.round);
          } catch {
            console.error("Error parsing round JSON:", match.round);
            return acc;
          }
        } else {
          round = match.round ?? {};
        ...
        if (roundNum > curRound) return acc;
      ...
      const groupedRounds = Object.values(groupedMatches)
        .map((roundGroup) => {
      ...
      if (groupedRounds.length === 1) {
        data.matches = groupedRounds[0].matches;
        ...
        data.rounds = groupedRounds;
        matchesMain.html(templateRound(data));
      }
    } else {
      data.matches = matches.sort((a, b) => b.match_time - a.match_time);
      if (data.matches.length > 0) {
        matchesMain.html(template(data));
      } else {
        matchesMain.html(mEerortemplate);
