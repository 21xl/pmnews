...
import templateSource from "../../templates/fav-template.html";
...
jQuery(document).ready(function ($) {
  ...
  async function fetchAndRenderMatches(date = null, tab = null) {
    try {
      ...
      const response = await fetch("/wp-json/sports/v1/favorite_matches", {
        method: "POST",
        headers: {
  ...
  Handlebars.registerHelper("winnerClass", function (homeScores, awayScores) {
    // Проверяем и парсим homeScores, если это строка
    if (typeof homeScores === "string") {
      try {
        homeScores = JSON.parse(homeScores);
      } catch (error) {
        console.error("Ошибка парсинга homeScores:", error);
        return ""; // Возвращаем пустой класс при ошибке
      }
    }

    // Проверяем и парсим awayScores, если это строка
    ...
    // Сравниваем значения, если они существуют
  ...
  Handlebars.registerHelper("displayScore", function (status, scores) {
    ...
    if (typeof scores === "string") {
      try {
        ...
        return "-"; // Возвращаем дефолтное значение при ошибке
      }
    }
...
function toggleFav(matchId, button, activate) {
  ...
  if (activate && index === -1) {
    ...
    // Найти блоки до удаления
    const leagueBlock = button.closest(".matches__ligue");
    const matchItem = button.closest(".matches__item");

    // Проверить, нужно ли отключить кнопку лиги
    const matchItems = leagueBlock.querySelectorAll(".matches__item");
    const anyFav = Array.from(matchItems).some((item) =>
      item.querySelector(".fav.addfav").classList.contains("active")
    );

    // Обновить состояние кнопки лиги
    ...
    // Проверить, остались ли матчи в лиге
    const remainingMatches = leagueBlock.querySelectorAll(".matches__item");
  ...
  if (document.querySelectorAll(".matches__item").length === 0) {
    const matchesMain = document.querySelectorAll(".matches__main")[0];

    if (matchesMain) return (matchesMain.innerHTML = mEerortemplate());
  }
}
...
function getAllFavorites() {
  ...
  for (let i = 0, len = localStorage.length; i < len; i++) {
    ...
    if (key.startsWith(prefix)) {
      try {
        ...
        // Добавляем только если data - массив и не пустой
        if (Array.isArray(data) && data.length > 0) {
          result.push({
            key: key.slice(prefix.length), // Удаляем префикс
            data: data,
          });
        }
      } catch {}
  ...
  // Возвращаем null, если массив result пуст
